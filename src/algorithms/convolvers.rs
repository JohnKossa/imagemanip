...
pub fn readonly_sub_grid_size_x_with_offset<T: Send>(func: fn(&Image, u32, u32) -> T, size: u32, offset_x: u32, offset_y: u32, img: &Image) -> Vec<T>{
    let img_width = img.get_width();
    let img_height = img.get_height();
    let mut ret_val: Vec<T> = Vec::new();
...
pub fn readonly_convolve_size_x<T: Send>(func: fn(&Image, u32, u32) -> T, size: u32, img: &Image) -> Vec<T>{
    let img_width = img.get_width();
    let img_height = img.get_height();
    let mut ret_val: Vec<T> = Vec::new();
    let mut coords: Vec<(u32,u32)> = Vec::new();
